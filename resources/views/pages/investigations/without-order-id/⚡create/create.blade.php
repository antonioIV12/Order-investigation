<div class="max-w-4xl mx-auto my-10 font-sans text-zinc-800 dark:text-zinc-200">
    <div class=" items-start mb-8">
        <div class=" flex flex-row justify-between">
            <h2 class="text-3xl font-black text-zinc-900 dark:text-white tracking-tight">Order Email Investigation</h2>
              <div
        ...
        <p class="text-zinc-500 text-sm mt-1">Search orders by Order Name and Email.</p>
    </div>
    <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 p-8">
        ...
        <div class="flex items-end gap-3 mb-8">
            <div class="flex flex-col w-full">
                <label class="text-[.625rem] font-bold text-zinc-400 uppercase tracking-widest ml-2 mb-2 block">Active
                    Profile</label>
                <div class="flex flex-row justify-between w-full">
                    <select wire:model.live="selectedIntegrationId" wire:key="select-integration"
                        ...
                        @foreach ($this->integrations as $int)
                            <option value="{{ $int->id }}">{{ $int->cp_slug }}</option>
                        @endforeach
                    </select>
                    <button type="button" wire:click="start" wire:key="run-button" wire:loading.attr="disabled"
        ...
        <div class="mb-10">
            ...
            <div
                ...
                <input type="file" wire:model="uploadedFile" id="file-upload" class="hidden">
                <label for="file-upload" class="cursor-pointer flex flex-col items-center">
                    <div
                        class="w-12 h-12 bg-zinc-100 dark:bg-zinc-900 rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
        ...
        <div wire:key="action-button-container"
                ...
                <div class="flex justify-between items-end mb-2">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-[10px] text-zinc-400 dark:text-zinc-500 font-bold uppercase">
                            {{ $processedCount }} / {{ $total }} COMPLETED</p>
                    </div>
                </div>
            ...
            @else
                <button wire:click="start" wire:loading.attr="disabled"
                    ...
                    <span>{{ $isProcessing ? 'Processing' : 'Run Process' }}</span>
                </button>
            @endif
        </div>
    </div>
    ...
    <script>
        document.addEventListener('livewire:init', () => {
            Livewire.on('process-next-item', () => {
                // Use $wire directly instead of @this
                @this.processNext();
            });
        });
    </script>
